...
function b_sitemap_mytube(){
	...
	$mytubeModule = $MyModule -> getByDirname('mytube');

	$MyConfig = &xoops_gethandler('config');
	$xoopstubeConfig = $MyConfig -> getConfigsByCat(0, $mytubeModule->getVar('mid'));

	$groups = is_object( $xoopsUser ) ? $xoopsUser -> getGroups() : XOOPS_GROUP_ANONYMOUS;
	$gperm_handler = &xoops_gethandler( 'groupperm' );

	$sitemap = array();
	$sql = "SELECT * FROM " . $xoopsDB -> prefix( 'xoopstube_cat' ) . " WHERE pid=0 ORDER BY title";
	$result = $xoopsDB -> queryF( $sql );
	while ( $myrow = $xoopsDB -> fetchArray( $result ) ) {
		if ( $gperm_handler -> checkRight( 'XTubeCatPerm', $myrow['cid'], $groups, $mytubeModule -> getVar('mid') ) ) {
        	$i = $myrow['cid'];
			...
			if ( $sitemap_configs["show_subcategoris"] ) {
				...
				foreach( $arr as $key => $ele ) {
					if ( $gperm_handler -> checkRight( 'XTubeCatPerm', $ele['cid'], $groups, $mytubeModule -> getVar('mid') ) ) {
						$j = $key;
